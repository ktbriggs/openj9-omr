	...
	if (!_extensions->isEvacuatorEnabled()) {
		...
		if (_debugger.isDebugEnd()) {
			...
			omrtty_printf("%5llu  0   :  gc start; survivor{%llx %llx} tenure{%llx %llx} evacuate{%llx %llx} projection:%llx\n", _extensions->scavengerStats._gcCount,
					(uint64_t)_survivorSpaceBase, (uint64_t)_survivorSpaceTop, (uint64_t)_extensions->_tenureBase, (uint64_t)_extensions->_tenureBase + _extensions->_tenureSize,
					(uint64_t)_evacuateSpaceBase, (uint64_t)_evacuateSpaceTop, (uint64_t)_survivorSpaceTop - (uint64_t)_survivorSpaceBase);
#if defined(EVACUATOR_DEBUG)
		}
#endif /* defined(EVACUATOR_DEBUG) */
